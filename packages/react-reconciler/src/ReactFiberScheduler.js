...
// 下一个要渲染的fiberRoot的expiration time
...
function commitRoot(root: FiberRoot, finishedWork: Fiber): void {
  ...
  let firstEffect;
  // rootFiber上也有effect，只有第一次执行的时候会有
  // 将这个effect放在其effect链的尾部
...
// 当前的wip已经是子节点，从这个节点向上遍历，对HostComponent或HostText创建dom元素并构建dom树，或更新dom
// 遍历过程中，若节点包含兄弟节点，则返回这个兄弟节点，
// 若更新完成，且其包含下一个兄弟节点，则返回这个节点的兄弟节点，然后进入beginWork对这个兄弟节点的子树更新和创建fiber
// 若无兄弟节点，对其父节点进行创建或更新dom元素
// 同时会将当前节点的effect链添加到父节点的effect链中，若当前节点也为effectTag，则将当前节点wip放入父节点的effect链尾部
  ...
  while (true) {
    ...
    if ((workInProgress.effectTag & Incomplete) === NoEffect) {
      ...
      if (enableProfilerTimer) {
        ...
        // 对HostComponent或HostText创建dom元素并构建dom子树，或更新dom
      ...
      // 将当前节点的effect链拼接到父节点的effect链尾部
      // 若当前节点有effect，workInProgress.effectTag > PerformedWork，则将当前wip作为effect添加到父节点的effect链尾部
      if (
        returnFiber !== null &&
        // Do not append effects to parents if a sibling failed to complete
        (returnFiber.effectTag & Incomplete) === NoEffect
      ) {
        // 若父节点的effect链为空，则将当前的effect链放在父节点的effect链
        if (returnFiber.firstEffect === null) {
          returnFiber.firstEffect = workInProgress.firstEffect;
        }
        // 将当前effect链拼接到父节点的effect链尾部
        ...
        // 若当前的wip也有effect，则将其添加到父effect链尾部
      ...
      // 若当前节点有兄弟节点，则返回这个兄弟节点，然后开始渲染更新这个兄弟节点
      if (siblingFiber !== null) {
        return siblingFiber;
      } else if (returnFiber !== null) {  // 若没有下个兄弟节点，则继续执行父节点的complete
...
// 处理单个fiber节点，构建或更新以当前节点为顶点的fiber树和dom树
...
// 首先执行beginWork从当前节点开始，执行获取最新的子节点fiber信息，然后和旧的子节点进行diff，得到最新的fiber。
// 然后向下继续执行获取最新的子fiber信息，直到到了叶子节点
// 然后开始completeUnitOfWork，向上开始对节点类型为HostComponent或HostText的节点构建或更新dom子树，并对dom的props进行diff。得到最新的dom子树
// 在向上的过程中若节点有兄弟节点，则执行beginWork，直到叶子节点，然后继续completeUnitOfWork.
// 最后直到又返回rootFiber结束，此时已经完全构建完成的fiber树和dom树
...
function renderRoot(
  ...
  // 从rootFiber开始向下开始构建或更新fiber树，直到叶子节点
  // 然后向上构建或更新dom节点，若期间有兄弟节点，则继续相加构建fiber，不断循环
  do {
    try {
      workLoop(isYieldy);
    }
...
// 找到更新的fiber节点对应的rootFiber
// 使用renturn不断的查找的过程中，会将过程中的所有祖先fiber节点的childExpirationTime设置为expiration time（childExpirationTime为子树中优先级最高的expiration time）
// 同时alternate同步更新
function scheduleWorkToRoot(fiber: Fiber, expirationTime): FiberRoot | null {
  ...
  // fiber未更新过或优先级低，则重新设置expiration time
  if (
  ...
  let node = fiber.return; // 获取父fiber节点
  let root = null;
  if (node === null && fiber.tag === HostRoot) {  // 当前节点就是rootFiber
    root = fiber.stateNode;
  } else {  // 不断向上查找，并设置各节点的childExpirationTime
    while (node !== null) {
...
// 并将当前的fiberRoot插入到fiberRoot链表的最前端，下次将会首先执行
function scheduleWork(fiber: Fiber, expirationTime: ExpirationTime) {
  const root = scheduleWorkToRoot(fiber, expirationTime); // 找到fiber所在的fiberRoot
  if (root === null) {
    return;
  }

  // 若当前的fiberRoot的优先级高于原本需要执行更新的fiberRoot的优先级，
  // 则将其打断，重新设置执行顺序，将当前的fiberRoot放在最前面，首先执行
  if (
    !isWorking && // 当前没有更新进行
    nextRenderExpirationTime !== NoWork &&  // 有接下来需要更新的fiberRoot
    expirationTime < nextRenderExpirationTime // 当前的fiberRoot的优先级比原本需要更新的fiberRoot的优先级高
  ) {
  ...
  if (
    !isWorking || // 当前没有更新进行
    isCommitting || // 处于commit阶段
    nextRoot !== root // 下一个需要执行渲染的fiberRoot不是当前的fiberRoot
  ) {
    const rootExpirationTime = root.expirationTime;
    requestWork(root, rootExpirationTime);
  }
...
// scheduleRoot链表，这是个双向循环链表，fiberRoot中还包含nextScheduledRoot和prevScheduledRoot
let firstScheduledRoot: FiberRoot | null = null;  // 表头的fiberRoot
let lastScheduledRoot: FiberRoot | null = null; // 表尾的fiberRoot

// 记录请求ReactScheduler的时候用的过期时间，
// 如果在一次调度期间有新的调度请求进来了，而且优先级更高，那么需要取消上一次请求，如果更低则无需再次请求调度。
...
let originalStartTimeMs: number = now(); // 系统当前时间
// 当前渲染时间，将当前时间转换为expiration time
...
function scheduleCallbackWithExpirationTime(
  ...
  if (callbackExpirationTime !== NoWork) { // 已经有一个callback在调度
    if (expirationTime > callbackExpirationTime) {  // 若当前的callback的优先级低于正在调度的callback的优先级，则退出
      return;
    } else {  // 优先级高于正在调度的callback的优先级，则取消正在调度的callback的执行
      if (callbackID !== null) {
        cancelDeferredCallback(callbackID); // 取消callback的执行
      }
    }
  } else {  // 当前没有callback在执行，则开始为执行callback做准备
    startRequestCallbackTimer();
  }

  // 为执行callback做准备
  callbackExpirationTime = expirationTime;  // 将当前的expirationTime赋值给callbackExpirationTime，表示已经有callback开始执行
  // 计算过期时间，用于在执行requestIdleCallback时设置timeout
  // 防止主线程一直被占，低优先级的callback一直不能执行被饿死
  const currentMs = now() - originalStartTimeMs;
  const expirationTimeMs = expirationTimeToMs(expirationTime);
  const timeout = expirationTimeMs - currentMs;
  // 模拟requestIdleCallback，当浏览器在每帧空闲时执行
...
function requestWork(root: FiberRoot, expirationTime: ExpirationTime) {
  ...
  if (isRendering) { // 若已经有任务进入渲染阶段，则退出
    return;
  }

  if (isBatchingUpdates) {  // 开启了批量更新
...
// 将fiberRoot加入到sheduleRoot 双向循环链表中
function addRootToSchedule(root: FiberRoot, expirationTime: ExpirationTime) {

  if (root.nextScheduledRoot === null) { // 若root的nextScheduledRoot为空，说明这个root没有在这个链表中，也就是从没有执行更新过
    // This root is not already scheduled. Add it.
    root.expirationTime = expirationTime;
    if (lastScheduledRoot === null) { // 链表为空，添加root
      firstScheduledRoot = lastScheduledRoot = root;
      root.nextScheduledRoot = root;
    } else {// 链表不为空，将root添加到链表最后
      lastScheduledRoot.nextScheduledRoot = root;
      lastScheduledRoot = root;
      lastScheduledRoot.nextScheduledRoot = firstScheduledRoot;
    }
  } else { // root已经存在于这个链表中，若root的优先级比之前的高，则更新优先级
...
function findHighestPriorityRoot() {
  ...
  if (lastScheduledRoot !== null) { // 链表中数据不为空
    let previousScheduledRoot = lastScheduledRoot;
    let root = firstScheduledRoot;
    while (root !== null) {
      const remainingExpirationTime = root.expirationTime;
      if (remainingExpirationTime === NoWork) { // 此时root中已经没有任务需要去执行，需要将这个root移出
        ...
        // 分别根据当前root在链表中的头、中、尾进行删除
        if (root === root.nextScheduledRoot) { // 说明链表中只有一个fiberRoot
          // This is the only root in the list.
          root.nextScheduledRoot = null;
          firstScheduledRoot = lastScheduledRoot = null;
          break;
        } else if (root === firstScheduledRoot) {
          // 当前的fiberRoot是sheduleRoot链表中的第一个，则将当前的fiberRoot移出链表
          const next = root.nextScheduledRoot;
          firstScheduledRoot = next;
          lastScheduledRoot.nextScheduledRoot = next;
          root.nextScheduledRoot = null;
        } else if (root === lastScheduledRoot) {
          // 当前的fiberRoot是sheduleRoot链表中的最后一个，则将当前的fiberRoot移出链表
          lastScheduledRoot = previousScheduledRoot;
          lastScheduledRoot.nextScheduledRoot = firstScheduledRoot;
          root.nextScheduledRoot = null;
          break;
        } else {
          // 当前的fiberRoot处于sheduleRoot链表中的中间的位置，移出链表
          previousScheduledRoot.nextScheduledRoot = root.nextScheduledRoot;
          root.nextScheduledRoot = null;
        }
        root = previousScheduledRoot.nextScheduledRoot;
      } else {// 当前root中还有任务去执行
        // 若当前的root的优先级为最高的，则对highestPriorityWork，highestPriorityRoot赋值
        if (
  ...
  nextFlushedRoot = highestPriorityRoot;  // 用来标志下一个需要渲染的root
  nextFlushedExpirationTime = highestPriorityWork; // 用来标志下一个需要渲染和对应的expirtaionTime
}

// 类似于requestIdleCallback中的回调函数，只在浏览器在每帧的空闲时执行
function performAsyncWork(dl) {
  if (dl.didTimeout) {
    // 回调超时。这意味着至少有一个更新过期。
    // 对rootSchedule进行迭代。如果它们包含过期的工作，就将下一个渲染过期时间设置为当前时间。
    // 这样做的效果是能够批量刷新所有过期的工作，而不是一次刷新每个级别。
    if (firstScheduledRoot !== null) {
      recomputeCurrentRendererTime(); // 重新计算当前render时间
